...
impl ActionableSelector {
  ...
  pub fn from_string<'a, T: Into<Cow<'a, str>>>(source: T) -> ScandentResult<ActionableSelector> {
    let selector = Selector::from_string(source)?;
    Ok(ActionableSelector::from_selector(selector))
  }

  pub fn _inverse(&self) -> ActionableSelector {
...
struct ActionableStep {
  axis: Axis,
  condition: Box<dyn Fn(Node) -> bool + Sync>,
}

impl ActionableStep {
  fn from_step(step: Step) -> ActionableStep {
    let mut conditions: Vec<Box<dyn Fn(Node) -> bool + Sync>> = vec![];
    ...
    for attr_req in attr_requirements {
      ...
      let localname_filter: Box<dyn Fn(&str) -> bool + Sync> = match attr_name_req.localname {
        Some(localname) => Box::from(move |name: &str| name == localname),
        None => Box::from(|_: &str| true),
      };
      let namespace_filter: Box<dyn Fn(Option<&'_ str>, Option<&'_ str>) -> bool + Sync> =
      ...
      let op_condition_function: Box<dyn Fn(&Attribute) -> bool + Sync> = match attr_op {
